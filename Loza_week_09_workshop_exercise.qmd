...
```{python}
total_population = population.query('indicator == "SP.POP.TOTL"')

population_long = total_population.melt(
    id_vars=["country", "indicator"], var_name="year", value_name="population"
)
...
hiv_data_mini = hiv_data_combined_pop.query('country == ["Ethiopia", "Uganda", "South Africa"]')
...
px.line(hiv_data_mini, x='year', y='incidence_rate', color='country', title='HIV incidence rates over time')
```
...
```{python}
px.line(hiv_data_mini, x='year', y='prevalence_rate', color='country', title='HIV prevalence rates over time')
```
...
hiv_data_combined_pop["continent"] = coco.convert(
    hiv_data_combined_pop["country_code"], to="continent"
)

hiv_data_combined_pop

hiv_data_2011 = hiv_data_combined_pop.query('year == "2011"')

hiv_data_2011

highest_by_continent = hiv_data_2011.loc[
    hiv_data_2011.groupby("continent")["prevalence_rate"].idxmax(),
    ["continent", "country", "prevalence_rate"],
]


highest_by_continent
