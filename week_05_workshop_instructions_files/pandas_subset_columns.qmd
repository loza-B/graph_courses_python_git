...
```{python}
# this code does not work yao["age", "sex"]
```
...
Use the [] operators to select the weight_kg and 
height_cm variables in the yao dataframe. Assign 
the result to a new dataframe called yao_weight_height. 
...
Assign the result to a new data frame called yao_no_education_consultation.


```{python}
yao_no_education_consultatio = yao.drop(columns=["highest_education", "consultation"])

yao_no_education_consultatio
```
...
Select all columns in the yao dataframe that 
start with "is_". Assign the result to a new 
data frame called yao_is_columns. 



```{python}
yao_is_column = yao.filter(regex="is_")
yao_is_column
```
...
dataframe to age_cat. Assign the result to a new
...
Consider the dataframe defined below. Use the str. replace method to clean 
the column names. 
```{python}
