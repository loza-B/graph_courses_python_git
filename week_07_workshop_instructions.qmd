...
```{python}
jobs.to_csv("data/jobs_years_required.csv", index=False)
```
...
px.scatter(
    ...
    labels={"midpoint_salary_estimate": "Midpoint salary", "years_required": "Years Required"})
```
# An increased number of years of experience required does not neccessarily positively correlate with a higher midpoint salary. 
...
def prog_lang(description):
    prompt = f"""
    Extract the programming languages mentioned in the job description. Specifically, return one of four categories regarding the languages mentioned: "R", "Python", "both", or "neither". If neither R or Python are mentioned or if no programming language is mentioned, return 'neither'. If both R or Python are mentioned return "both". If only R is mentioned, return "R". If only Python is mentioned, return "Python". Do not return any other outputs. 
    Here's the job description:
    {description}
    """
    return llm_chat(prompt)
...
px.box(
    ...
    x = "lang_req", 
    y = "midpoint_salary_estimate",
    labels={"midpoint_salary_estimate": "Midpoint salary", "lang_req": "Programming language requirement"}
)
```

Jobs requiring python only had the highest median midpoint salary. Jobs asking for R only had the lowest median midpoint salary. The largest variation in midpoint salary was for jobs requiring neither python or R. 
...
def tech_skills(description):
    prompt = f"""
    Extract the most common technical skillmentioned in the job description. 
    - Only include concrete tehcnical skills and exclude soft skills such as communication, team player, motivated. Normalize variations of the same words. 
    - Exclude knowledge of programming languages. If no technical skills are determined from the description, return none. 
    - Return only one skill per job. Return only the skill and not a sentence descrbing the skill. The one skill should be the most common one among all jobs. 
    - Group skills with similar meanings together. All output should be returned in English. 
    - Group "Data analysis" "data analysis", "data analytics", "Data Analytics" and "Data Analysis" together into one category. 
    - Remove all other repetitions such as Data Analytics, Data Analysis and Data analytics. 
    - Group "Data management" and "Database management" together.
    Here's the job description:
    {description}
    """
    return llm_chat(prompt)
...
# Apply the function to the dataset
jobs["tech_skills"] = tech_skills_vec (jobs["job_description"])
...
px.histogram(
    jobs,
    x= "tech_skills", 
    title="Most common technical skills in Data analysit jobs",
    labels={"tech_skills": "Technical Skills"})
```
